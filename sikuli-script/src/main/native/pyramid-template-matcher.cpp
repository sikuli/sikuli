 ...
 *  Copyright 2010 Project Sikuli. All rights reserved.
 *
 */
...
{ 
   ...
   Size targetSize = target.size();
	...
	if (levels > 0){
		...
		cout << "[" << sourceSize.width << " x " << sourceSize.height << "] ";
		cout << "[" << targetSize.width << " x " << targetSize.height << "]" << endl;
...
#else
		matchTemplate(source,target,result,CV_TM_CCORR_NORMED);
#endif
		delete t;
